...
export default function CheckInScreen() {
  ...
  const handleCheckIn = async () => {
    ...
    try {
      ...
      const alreadyCheckedIn = await checkSessionManually(event, fullName, resolvedRole, router);
      ...
      await SecureStore.setItemAsync(`checkedIn_${secureKeyName}_${secureKeyEvent}`, 'true');
      await SecureStore.setItemAsync(`checkInTime_${secureKeyName}_${secureKeyEvent}`, Date.now().toString());
      ...
      if (resolvedRole === 'teamlead') {
        ...
        router.push({ pathname: '/teamlead/dashboard', params: { event, name: fullName } });
      } else {
        router.push({ pathname: '/volunteer/dashboard', params: { event, name: fullName } });
      }
    } catch (err) {
      console.error(err);
